...
public class PlayMusicService extends Service {
    ...
    private float sumVolToDecrease =  0;
    ...
    private void startTimerCountDown(long millisInFuture) {
        ...
        timer = new CountDownTimer(millisInFuture, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                if( isSleepMode && audioManager != null  ) {

                    int currentVolume = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);

                    if( currentVolume <= 0 ) return;
                    int totalSteps = (int) (millisUntilFinished / 1000);

                    sumVolToDecrease += (float)currentVolume / totalSteps;

                    Log.i("Volumn", String.valueOf(sumVolToDecrease));
                    Log.i("millisUntilFinished", String.valueOf(millisUntilFinished));

                    if( sumVolToDecrease >= 1 ){
                        currentVolume -= 1;
                        sumVolToDecrease = 0;
                        audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, currentVolume, AudioManager.FLAG_SHOW_UI);
                        // show man hinh giam am
//                        audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, currentVolume, -1);
        ...
        if (mediaPlayer == null) {
            ...
            mediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                ...
                public void onCompletion(MediaPlayer mp) {
                    if (currentSong != null && mp.getCurrentPosition() > 0) {
    ...
    private void createNotification(Song currentSong) {
        ...
        notificationBuilder = new NotificationCompat.Builder(this, CHANNEL_ID_MUSIC)
                ...
                .setContentTitle(currentSong != null ? currentSong.getTitle() : "")
                .setContentText(currentSong != null  ? currentSong.getUserUpload().getUsername() : "")
    ...
    private void handleChangeStatus(int actionCode) {
        switch (actionCode) {
            ...
            case Constant.ACTION_START_SLEEP_MODE: {
                isSleepMode = true;
                break;
            }
//            default: {
...
//            }
    ...
    private void sendBroadcastToActivity(int action) {
        ...
        bundle.putBoolean(Constant.SLEEP_MODE, isSleepMode);
        ...
        firebaseService.changeCurrentSong(song, new FirebaseService.OnSaveCompleteListener() {
            ...
            public void onSaveComplete(boolean isSuccess) {
                if( isSuccess ) {
                    ...
                    Intent intent = new Intent(context, PlayMusicService.class);
                    intent.putExtra(Constant.ACTION_TYPE, Constant.ACTION_PLAY);
                    intent.putExtras(bundle);
                    // Start the service
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static void startTimer(Context context, long time, View view) {
        Snackbar.make(view, "Nhạc sẽ tắt sau " + Util.convertDurationToString(time/1000) +" phút nữa!", Snackbar.LENGTH_SHORT).show();
    ...
    public static void startSleepMode(Context context, long time, View view) {
        Snackbar.make(view, "Nghe nhạc để ngủ bắt đầu bật!", Snackbar.LENGTH_SHORT).show();

        Bundle bundle = new Bundle();
        bundle.putLong(Constant.TIMER_COUNT,time);
        Intent intent = new Intent(context, PlayMusicService.class);
        intent.putExtras(bundle);
        intent.putExtra(Constant.ACTION_TYPE, Constant.ACTION_START_SLEEP_MODE);
        // Start the service
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            context.startForegroundService(intent);
        } else {
    ...
    public static void cancelTimer(Context context, View view) {
        Snackbar.make(view, "Đã tắt hẹn giờ", Snackbar.LENGTH_SHORT).show();
